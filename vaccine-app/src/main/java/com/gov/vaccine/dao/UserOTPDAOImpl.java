...
public class UserOTPDAOImpl implements UserOTPDAO {
	...
	@Override
	public boolean updateOtpByEmailID(String emailID, int newOtp) {
		System.out.println("Invoked updateOtpByEmailID()");
		Session session = null;
		try {
			session = factory.openSession();
			session.beginTransaction();
			Query query = session.getNamedQuery("updateOtpByEmailID");
			query.setParameter("EMAILID", emailID);
			query.setParameter("newOtp", newOtp);
